...
Lev_4_Window::Lev_4_Window(QWidget *parent) : QMainWindow(parent)
{
    ...
    Subject_Area->move(desktop_width/4,desktop_height/40);

    Masha   = new Static_Object(":/res4/vnuchka-111.png", desktop_width/2,desktop_height/2, this);
    Masha->move(desktop_width/60, desktop_height-Masha->height());
    Masha->show();

    Mysl    = new Static_Object(":/res2/mysl.png", desktop_width/4,desktop_height/4, this);
    Mysl->move(Masha->x()+Masha->width()/4, Masha->y()-Masha->height()/2);
    Mysl->show();
    ...
    Holst   = new Static_Object(":/res4/holst.png", desktop_width*0.5, desktop_height*0.8, this);
    ...
    for(int i=0; i<8; ++i)
    {
            ...
            if (i==3||i==7)
            {
                Mysl_Object[i].resize_obj(desktop_width/15,desktop_height/15);
            } else
            {
                Mysl_Object[i].resize_obj(desktop_width/12,desktop_height/12);
            }
            Mysl_Object[i].move(Mysl->width()/2-Mysl_Object[i].width()/2, Mysl->height()/2-Mysl_Object[i].height());
            Mysl_Object[i].hide();
            Pen[i].resize_obj(100,100);
            Pen[i].setParent(this);
            Vnuchka[i].setParent(this);
            Vnuchka[i].resize_obj(desktop_width/3,desktop_height/3);
    ...
    Hand1   = new Static_Object(":/res/ruka.png", desktop_width/12,desktop_height/12, this);
    ...
    while(i<8)                                          // пока не установлены координаты всех продуктов выполняем цикл
    {
        ...
        for(int j=i; j>=1; --j)                         // для j от i пока не станет равным 1
        {
            ...
            else if(j==1)                               // иначе, если совпадений нет, то
            {
                ...
                {
                case 1: x[i] = Subject_Area->x()+z;     y[i]=Subject_Area->y(); break;
                case 2: x[i] = Subject_Area->x()+z*2;   y[i]=Subject_Area->y(); break;
                case 3: x[i] = Subject_Area->x()+z*3;   y[i]=Subject_Area->y(); break;
                case 4: x[i] = Subject_Area->x()+z*4;   y[i]=Subject_Area->y(); break;
                case 5: x[i] = Subject_Area->x()+z*5;   y[i]=Subject_Area->y(); break;
                case 6: x[i] = Subject_Area->x()+z*6;   y[i]=Subject_Area->y(); break;
                case 7: x[i] = Subject_Area->x()+z*7;   y[i]=Subject_Area->y(); break;
                }
...
void Lev_4_Window::mouseMoveEvent(QMouseEvent *pe)
{
    ...
    int d = pe->x();
    int f = Vnuchka[0].x();
    int g = Vnuchka[0].x()+Vnuchka[0].width();
    Background->setText(QString::number(d)+"___"+QString::number(f)+"___"+QString::number(g));
}
