...
Lev_3_Window::Lev_3_Window(QWidget *parent) : QMainWindow(parent)
{
    ...
    Button_Return->move(desktop_width/40,desktop_height/40);
    Button_Return->resize(desktop_width/20,desktop_height/14);
    Button_Return->raise();

    Stol = new Static_Object(":/res3/stol.png", desktop_width/2, desktop_height/4, this);
    Stol->move(desktop_width/2-Stol->width()/8, desktop_height-Stol->height()-Stol->height()/10);
    Stol->raise();

    Babka = new Static_Object(":/res3/babka.png", desktop_width/2, desktop_height/2, this);
    Babka->move(desktop_width-Babka->width()+Babka->width()/2, desktop_height-Babka->height());
    Babka->raise();
    Mysl = new Static_Object(":/res2/mysl.png", desktop_width/2, desktop_height/4, this);
    Mysl->move(Babka->x()+Babka->width()/6, Babka->y()-Babka->height()/2);

    Produkt     = new Static_Object[8];
    Produkt[0].pic_obj(":/res3/tazik-01.png");
    Produkt[0].resize_obj(desktop_width/13,desktop_height/13);
    Produkt[0].move(15, desktop_height-Produkt[0].height()*2);
    Produkt[1].pic_obj(":/res3/milk.png");
    Produkt[1].resize_obj(desktop_width/12,desktop_height/12);
    Produkt[2].pic_obj(":/res3/muka.png");                  // Инициализируем массив продуктов
    Produkt[2].resize_obj(desktop_width/12,desktop_height/12);
    Produkt[3].pic_obj(":/res3/egs.png");
    Produkt[3].resize_obj(desktop_width/18,desktop_height/18);
    Produkt[4].pic_obj(":/res3/oil.png");
    Produkt[4].resize_obj(desktop_width/12,desktop_height/12);
    Produkt[5].pic_obj(":/res3/sol.png");
    Produkt[5].resize_obj(desktop_width/16,desktop_height/16);
    Produkt[6].pic_obj(":/res3/venchik.png");
    Produkt[6].resize_obj(desktop_width/12,desktop_height/12);
    ...
    Produkt_Babka[0].resize_obj(desktop_width/13,desktop_height/13);
    Produkt_Babka[0].move(Mysl->x()+Mysl->width()/9, Mysl->y()+Mysl->height()/8);
    Produkt_Babka[1].pic_obj(":/res3/milk.png");
    Produkt_Babka[1].resize_obj(desktop_width/12,desktop_height/12);
    Produkt_Babka[2].pic_obj(":/res3/muka.png");
    Produkt_Babka[2].resize_obj(desktop_width/12,desktop_height/12);
    Produkt_Babka[3].pic_obj(":/res3/egs.png");             // Массив продуктов о которых думает бабка
    Produkt_Babka[3].resize_obj(desktop_width/18,desktop_height/18);
    Produkt_Babka[4].pic_obj(":/res3/oil.png");
    Produkt_Babka[4].resize_obj(desktop_width/12,desktop_height/12);
    Produkt_Babka[5].pic_obj(":/res3/sol.png");
    Produkt_Babka[5].resize_obj(desktop_width/16,desktop_height/16);
    Produkt_Babka[6].pic_obj(":/res3/venchik.png");
    Produkt_Babka[6].resize_obj(desktop_width/12,desktop_height/12);
    Produkt_Babka[7].pic_obj(":/res3/pechka.png");
    Produkt_Babka[7].resize_obj(desktop_width/12,desktop_height/12);
    ...
    for(int i=0; i<5; ++i)
    {
        ...
        Tazik[i].resize_obj(desktop_width/13,desktop_height/13);
        Tazik[i].hide();
    }

    Produkt_Babka[0].show();
    Hand    = new Static_Object(":/res/ruka.png", desktop_width/12, desktop_height/12, this);          // подсказка - рука
...
void Lev_3_Window::mouseMoveEvent(QMouseEvent *pe)
{
    if(!FLAG_KOLOBOK)                   // если тесто не готово, то перемещаем продукты
    {
        if(FLAG_PRODUKT)
        {
            Produkt[index].move(pe->x()-Produkt[index].width()/6, pe->y()-Produkt[index].height()/2);  // курсор по центру предмета
...
void Lev_3_Window::mouseReleaseEvent(QMouseEvent *pe)
{
    if(!FLAG_KOLOBOK)           // если тесто не готово, добавляем ингридиенты
    {
        if((pe->x()>Stol->x())&&(pe->x()<Stol->x()+Stol->width())&&                 // поставили ли предмет на стол
           (pe->y()>Stol->y()-Stol->height()/6)&&(pe->y()<Stol->y()+Stol->height()/4)&&FLAG_PRODUKT)
        {
            yes->setVolume(100);
            yes->play();
            ...
            {
                Produkt[0].move(Stol->x()+Tazik[0].width(), Stol->y()-Tazik[0].height()/2);
            }
            ...
            Produkt_Babka[index].move(Mysl->x()+Mysl->width()/6, Mysl->y()+Mysl->height()/4);           // обновляем мысли бабки - остальные
            Produkt_Babka[index].show();
            switch(index)                                                                               //   меняем содержимое тазика
            {
            case 2: Produkt[0].hide(); Tazik[0].move(Stol->x()+Tazik[0].width(), Stol->y()-Tazik[0].height()/2); Tazik[0].show(); break;
            case 3: Tazik[0].hide(); Tazik[1].move(Stol->x()+Tazik[1].width(), Stol->y()-Tazik[1].height()/2); Tazik[1].show(); break;
            case 4: Tazik[1].hide(); Tazik[2].move(Stol->x()+Tazik[2].width(), Stol->y()-Tazik[2].height()/2); Tazik[2].show(); break;
            case 5: Tazik[2].hide(); Tazik[3].move(Stol->x()+Tazik[3].width(), Stol->y()-Tazik[3].height()/2); Tazik[3].show(); break;
            case 6: Tazik[2].hide(); Tazik[3].move(Stol->x()+Tazik[3].width(), Stol->y()-Tazik[3].height()/2); Tazik[3].show(); break;
            case 7: Tazik[3].hide(); Tazik[4].move(Stol->x()+Tazik[4].width(), Stol->y()-Tazik[4].height()/2); Tazik[4].show(); break;
            }
...
void Lev_3_Window::help_game()
{
    ...
    x1 = Stol->x()+Stol->width()/6;
    y1 = Stol->y()+Stol->height()/8;
